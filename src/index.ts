...
app.post('/db/updateUser' ,async (c) => {
  ...
  const result = await updateUser(SUPABASE_URL, SUPABASE_API_KEY, name,newName);
  return c.json({ result: result});
})
export default app
...
async function updateUser(SUPABASE_URL: string, SUPABASE_API_KEY: string,name:String,newName:String){
  const supabase = createClient(SUPABASE_URL, SUPABASE_API_KEY);
